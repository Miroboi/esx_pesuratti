...
RegisterNetEvent('esx_pesuratti:onUse')
AddEventHandler('esx_pesuratti:onUse', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			if Config.IgnoreAbort then
				TriggerServerEvent('esx_pesuratti:removeKit')
			end
			TaskStartScenarioInPlace(playerPed, "WORLD_HUMAN_MAID_CLEAN", 0, true)

			Citizen.CreateThread(function()
				ThreadID = GetIdOfThisThread()
				CurrentAction = 'clean_vehicle'

				Citizen.Wait(Config.PuhdistusTime * 1000)
